...
export function PaymentSettingsForm({ initialSettings, runtimeProvider }: { initialSettings: Record<string, any>; runtimeProvider: string }) {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><CheckCircle2 className={`size-5 ${runtimeProvider === 'mock' ? 'text-amber-600' : 'text-emerald-600'}`} /><p className="font-semibold">{runtimeProvider === 'mock' ? 'Simulação local ativa' : 'AbacatePay ativo'}</p></div><p className="mt-1 text-sm text-slate-500">As taxas abaixo alimentam a precificação e os relatórios. A tarifa efetiva vem do webhook.</p></div><div className="flex items-center gap-3 rounded-xl bg-slate-50 px-4 py-3"><div><p className="text-sm font-semibold">Repasses reais</p><p className="text-xs text-slate-500">Chave de segurança operacional</p></div><Switch checked={Boolean(settings.payouts_enabled)} onCheckedChange={(checked) => setSettings((current: any) => ({ ...current, payouts_enabled: checked }))} aria-label="Habilitar repasses reais" /></div></div>
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{fields.map((field) => { const Icon = field.icon; 

return <label key={field.key} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between"><div className="flex size-10 items-center justify-center rounded-xl bg-violet-50 text-violet-700"><Icon className="size-5" /></div><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600">{field.suffix}</span></div><Label htmlFor={field.key} className="mt-4 block text-base">{field.label}</Label><p className="mt-1 min-h-10 text-xs leading-5 text-slate-500">{field.helper}</p><Input id={field.key} inputMode="decimal" type="number" min="0" step="0.01" className="mt-3 text-lg font-semibold" value={settings[field.key] ?? 0} onChange={(event) => setSettings((current: any) => ({ ...current, [field.key]: event.target.value }))} /></label>; })}</div>
    <div className="sticky bottom-4 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-xl backdrop-blur sm:flex-row sm:items-center sm:justify-between"><p role="status" className="text-sm text-slate-600">{feedback ?? 'Revise os valores antes de publicar uma alteração.'}</p><Button onClick={save} disabled={saving}><Save className="mr-2 size-4" />{saving ? 'Salvando…' : 'Salvar configurações'}</Button></div>
  </div>;
}
