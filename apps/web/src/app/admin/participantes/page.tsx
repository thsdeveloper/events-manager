...
export default function ParticipantesPage() {
	...
	useEffect(() => {
		async function loadFilterOptions() {
			try {
				const response = await fetch('/api/admin/participantes/filter-options', { credentials: 'include' });
				...
				if (response.ok) {
					...
					setError(problem?.detail ?? 'Erro ao carregar opções de filtro');
				}
			} catch {
	...
	useEffect(() => {
		...
		async function loadData() {
			...
			try {
				...
				const response = await fetch(`/api/admin/participantes?${params.toString()}`, { credentials: 'include' });
				...
				if (!response.ok) {
					setError(json.detail ?? 'Erro ao carregar participantes');
				} else {
					setData(json);
				}
			} catch {
	...
	}, [currentPage, filtersKey, handleUnauthorized, refreshTrigger]);
	...
	return (
		<div className="space-y-6">
			...
			<div className="flex items-center gap-4">
				...
				<div>
					<h1 className="text-3xl font-bold text-gray-900 dark:text-white">Gerenciar Participantes</h1>
