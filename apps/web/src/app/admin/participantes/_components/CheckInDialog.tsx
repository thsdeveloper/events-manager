...
export function CheckInDialog({ participant, open, onOpenChange, onSuccess }: CheckInDialogProps) {
	...
	const handleCheckIn = async () => {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(data.detail || 'Erro ao realizar check-in');
			}
	...
	const handleUndoCheckIn = async () => {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(data.detail || 'Erro ao desfazer check-in');
			}
					...
					<div className="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
						<div className="space-y-2">
							<div>
								...
								<p className="text-base font-semibold text-gray-900 dark:text-white">{participant.participant_name}</p>
							</div>
							<div>
							...
							<div>
								...
								<p className="font-mono text-sm text-gray-900 dark:text-white">{participant.ticket_code}</p>
							</div>
						</div>
					</div>
