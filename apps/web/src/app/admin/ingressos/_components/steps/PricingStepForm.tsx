			...
			<FormField
				...
				render={({ field }) => (
					<FormItem>
						...
						<FormDescription>Valor que você deseja receber por ingresso</FormDescription>
						<FormMessage />
					</FormItem>
				)}
			/>
					...
					<FormItem className="space-y-3">
						...
						<FormControl>
							<RadioGroup onValueChange={field.onChange} value={field.value ?? 'passed_to_buyer'} className="space-y-3">
								<label
									className={`flex items-start gap-3 p-4 border-2 rounded-lg cursor-pointer hover:bg-accent/50 transition-colors ${
										field.value === 'passed_to_buyer' ? 'border-primary bg-primary/5' : 'border-border'
									}`}
								>
									...
									<div className="flex-1">
										...
										<p className="text-sm text-muted-foreground mt-1">
											Você recebe quase o valor total. A taxa de conveniência (~{convenienceFeePercentage.toFixed(2)}%)
											é cobrada do comprador
										</p>
												...
												<div className="space-y-2 text-sm">
													...
													<div className="space-y-1">
														<div className="flex justify-between text-xs text-muted-foreground">
															<span>
																Taxa AbacatePay ({feeConfig.providerPercentageFee}% de {formatCurrency(fees.buyerPrice)}{' '}
														...
														<div className="flex justify-between text-xs text-muted-foreground">
															<span>
																Taxa Plataforma ({feeConfig.platformFeePercentage}% de{' '}
																{formatCurrency(fees.ticketPrice)}):
															</span>
					...
					render={({ field }) => (
						<FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
							<div className="space-y-0.5">
								...
								<FormDescription>Compradores poderão parcelar sem juros</FormDescription>
							</div>
							<FormControl>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormDescription>Ingressos abaixo deste valor não poderão ser parcelados</FormDescription>
									<FormMessage />
								</FormItem>
							)}
						/>
