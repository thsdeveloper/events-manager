...
export default function IngressosPage() {
	...
	useEffect(() => {
		async function loadFilterOptions() {
			try {
				const response = await fetch('/api/admin/ingressos/filter-options', { credentials: 'include' });

				if (response.status === 401) {
					router.replace('/login?redirect=/admin/ingressos');
				...
				if (response.ok) {
					...
					throw new Error(problem?.detail ?? 'Erro ao carregar opções de filtro');
				}
			} catch (cause) {
				setError(cause instanceof Error ? cause.message : 'Erro ao carregar opções de filtro');
			}
		}
	...
	useEffect(() => {
		...
		async function loadData() {
			...
			try {
				...
				const response = await fetch(`/api/admin/ingressos?${params.toString()}`, { credentials: 'include' });
				...
				if (!response.ok) {
					setError(json.detail ?? 'Erro ao carregar ingressos');
				} else {
					setData(json);
				}
			} catch {
	...
	}, [currentPage, filtersKey, refreshTrigger, router]);
		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-4">
					<Link href="/admin" className="rounded-lg p-2 transition-colors hover:bg-gray-100 dark:hover:bg-gray-700">
						<ArrowLeft className="size-5" />
					</Link>
					<div>
						<h1 className="text-3xl font-bold text-gray-900 dark:text-white">Gerenciar Ingressos</h1>
						<p className="mt-1 text-gray-600 dark:text-gray-400">Crie e gerencie ingressos para seus eventos</p>
					</div>
				</div>
			...
			{!error && (
				<div className="space-y-4">
					<div className="flex flex-col gap-4 sm:flex-row sm:items-center">
						...
						<TicketFilters filters={filters} onChange={handleFilterChange} eventOptions={eventOptions} />
					</div>
