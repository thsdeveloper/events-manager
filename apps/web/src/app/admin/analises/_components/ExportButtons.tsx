...
export function ExportButtons({
			...
			...data.map((row) =>
				headers
					.map((header) => {
						...
						const value = String(rawValue ?? '').replace(/^[=+\-@]/, (match) => `'${match}`);
	...
	const exportReportPDF = async () => {
		...
		try {
			...
			doc.text(`Gerado em ${format(new Date(), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })}`, pageWidth / 2, yPosition, {
				align: 'center',
			});
	...
	const exportFinancialSummary = async () => {
		...
		try {
			...
			doc.setFontSize(18);
			doc.setFont('helvetica', 'bold');
			doc.text('Resumo Financeiro Gerencial', pageWidth / 2, yPosition, { align: 'center' });

			yPosition += 10;
			doc.setFontSize(10);
			doc.setFont('helvetica', 'normal');
			doc.text(`Período: ${format(new Date(), 'MMMM yyyy', { locale: ptBR })}`, pageWidth / 2, yPosition, {
				align: 'center',
			});
			...
			autoTable(doc, {
				...
				body: [
					...
					['Ingressos Vendidos', kpiData.ticketsSold.toString()],
					['Participantes Únicos', kpiData.uniqueParticipants.toString()],
					['Ticket Médio', formatCurrency(kpiData.ticketsSold > 0 ? kpiData.totalRevenue / kpiData.ticketsSold : 0)],
				],
				theme: 'grid',
			...
			if (paymentMethodsData.length > 0) {
				...
				autoTable(doc, {
					startY: yPosition,
					head: [['Método', 'Transações', 'Receita Bruta']],
					body: paymentMethodsData.map((method) => [
						method.method,
						method.count.toString(),
						formatCurrency(method.revenue),
			...
			doc.text(
				'Resumo gerencial baseado nos filtros selecionados. Este documento não possui valor fiscal.',
				pageWidth / 2,
				doc.internal.pageSize.getHeight() - 15,
				{ align: 'center' },
			);

			doc.save(`resumo_financeiro_${format(new Date(), 'yyyy-MM-dd')}.pdf`);
		} finally {
			setIsExporting(false);
		}
	};
