...
export const organizerNavigation = [
  ...
  { name: 'Minha conta', href: '/admin/minha-conta', icon: UserCog },
];

export default function AdminSidebar({ organizer }: { organizer: OrganizerProfile }) {
  ...
  return <aside className="fixed inset-y-0 left-0 z-40 hidden w-72 flex-col border-r border-slate-200 bg-white lg:flex">
    <div className="border-b border-slate-200 p-5"><Link href="/admin/dashboard" className="flex items-center gap-3"><div className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-slate-950 text-white"><Building2 className="size-6" /></div><div className="min-w-0"><p className="truncate text-sm font-bold text-slate-950">{organizer.name}</p><p className="text-xs text-slate-500">Workspace do organizador</p></div></Link></div>
...
return <Link key={item.href} href={item.href} className={cn('flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-colors', isActive(item.href) ? 'bg-violet-50 text-violet-800' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950')}><Icon className={cn('size-5', isActive(item.href) ? 'text-violet-600' : 'text-slate-400')} />{item.name}</Link>; })}</nav>
    <div className="border-t border-slate-200 p-4"><Link href="/" className="flex items-center justify-center rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Ver site público</Link></div>
  </aside>;
}
