...
export const WizardProgressBar = memo(function WizardProgressBar({
  ...
  const progress = useMemo(() => Math.max(0, (currentStep / totalSteps) * 100), [currentStep, totalSteps]);
  const estimatedTime = useMemo(
    () => steps.slice(currentStep).reduce((sum, step) => sum + (step.estimatedMinutes || 0), 0),
    [steps, currentStep],
  );
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="relative pt-1">
        ...
        <ol className="relative flex justify-between gap-1.5">
