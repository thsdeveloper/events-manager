...
export default function EventCheckout({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (selectedInstallments && selectedInstallments > 1 && ticketData?.allow_installments) {
				await createInstallmentCheckout(selectedTicket, selectedInstallments, participantInfo);
			} else {
				// Pagamento à vista via AbacatePay
				await createCheckoutSession(selectedTickets, participantInfo);
			}
		} catch {
			// O hook mantém a mensagem visível no formulário.
		}
	};
	...
	return (
		<div className="max-w-4xl mx-auto p-6">
			...
			{!showParticipantForm ? (
				<TicketSelection eventId={eventId} tickets={tickets} onCheckout={handleTicketSelection} isLoading={isLoading} />
			) : (
