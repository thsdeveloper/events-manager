...
export class ContentService {
  ...
  async getSite() {
    const [settingsResult, mainResult, footerResult] = await Promise.all([
      this.database.from('site_settings').select('*').limit(1).maybeSingle(),
  ...
  async listPosts(limit: number, page: number) {
  ...
  private async getNavigation(id: string) {
    const { data: navigation, error } = await this.database
      .from('navigation')
      .select('*')
      .eq('id', id)
      .maybeSingle();
    if (error) throw error;
    if (!navigation) return { id, title: id, items: [] };
    ...
    const attachChildren = (item: any): any => ({ ...item, children: (byParent.get(item.id) ?? []).map(attachChildren) });

    return { ...navigation, items: (byParent.get(null) ?? []).map(attachChildren) };
  }

  private async getBlockItem(collection: string, id: string, postPage: number) {
    const table = blockTables[collection as keyof typeof blockTables];
    ...
    if (collection === 'block_hero' && data.button_group) {
      const { data: group } = await this.database.from('block_button_groups').select('*').eq('id', data.button_group).single();
    ...
    if (collection === 'block_pricing') {
      const { data: cards } = await this.database.from('block_pricing_cards').select('*').eq('pricing', id).order('sort');
    ...
    if (collection === 'block_events') {
      ...
      if (data.filter_featured) query = query.eq('featured', true);
      if (!data.show_past_events) query = query.gte('start_date', new Date().toISOString());
      if (data.filter_by_category) query = query.eq('category_id', data.filter_by_category);
