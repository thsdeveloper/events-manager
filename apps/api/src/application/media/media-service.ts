...
export interface StoredMedia {
	file: Record<string, unknown> & { id: string; filename: string };
	url: string;
}
...
export function hasValidFileSignature(mimetype: string, buffer: Buffer) {
...
export class MediaService {
	...
	async upload(input: MediaUpload) {
	...
	async getPublicUrl(id: string) {
		const url = await this.repository.findPublicUrl(id);
		if (!url) throw new ApiError('Arquivo não encontrado.', 404, 'MEDIA_NOT_FOUND');
		return url;
	}
}
