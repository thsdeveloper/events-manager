...
export async function uploadMedia(
  ...
  if (!allowedTypes.has(file.mimetype)) throw new ApiError('Tipo de arquivo não permitido.', 422, 'INVALID_FILE_TYPE');
  const buffer = await file.toBuffer();
  if (buffer.length > 20 * 1024 * 1024) throw new ApiError('O arquivo excede o limite de 20 MB.', 413, 'FILE_TOO_LARGE');
