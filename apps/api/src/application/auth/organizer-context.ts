...
export async function requireOrganizer(request: FastifyRequest, clients: SupabaseClients) {
...
export async function assertOrganizerOwnsEvent(clients: SupabaseClients, organizerId: string, eventId: string) {
...
export async function getOwnedRegistration(clients: SupabaseClients, organizerId: string, registrationId: string) {
  const { data, error } = await clients.admin
    ...
    .maybeSingle();
  if (error) throw error;
  if (!data) throw new ApiError('Inscrição não encontrada ou sem permissão.', 404, 'REGISTRATION_NOT_FOUND');
  return data;
}
