...
export function buildInstallmentAmounts(total: number, installments: number, minimumTotal = 0) {
	...
	return Array.from(
		{ length: installments },
		(_, index) => (index === installments - 1 ? baseInCents + remainder : baseInCents) / 100,
	);
}
