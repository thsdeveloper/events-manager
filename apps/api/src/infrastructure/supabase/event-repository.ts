import type { Event, EventInput } from '@events-manager/contracts';
import type { EventRepository } from '../../application/events/event-service.js';
...
export class SupabaseEventRepository implements EventRepository {
	...
	private async organizerForUser(userId: string, requireActive = false) {
	...
	async listForUser(userId: string) {
		...
		const { data, error } = await this.clients.admin
			.from('events')
			.select(
				'*,cover_image:media_files(*),category_id:event_categories(*),tickets:event_tickets(*),registrations:event_registrations(id,status,payment_status,quantity)',
			)
	...
	async createForUser(userId: string, input: EventInput) {
		const organizer = await this.organizerForUser(userId, true);
		if (!organizer) return 'organizer_required' as const;
		const { data, error } = await this.clients.admin
			.from('events')
	...
	async updateForUser(userId: string, id: string, input: Partial<EventInput>) {
		const organizer = await this.organizerForUser(userId);
		if (!organizer) return null;
		const { data, error } = await this.clients.admin
			.from('events')
