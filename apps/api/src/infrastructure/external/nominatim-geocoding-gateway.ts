import type { GeocodingGateway, PlacePrediction } from '../../application/external/external-service.js';
...
function toPrediction(place: NominatimPlace): PlacePrediction | null {
	const latitude = Number(place.lat);
	const longitude = Number(place.lon);
	if (!place.display_name || !Number.isFinite(latitude) || !Number.isFinite(longitude)) return null;
 ...
 */
export class NominatimGeocodingGateway implements GeocodingGateway {
	constructor(
	...
	private async request(path: string, params: Record<string, string>): Promise<unknown> {
		...
		if (response.status === 429) {
			throw new ApiError(
				'O serviço de endereços recusou a consulta por excesso de requisições. Tente novamente em instantes.',
				429,
				'PLACES_RATE_LIMITED',
			);
		}
	...
	async search(input: string): Promise<PlacePrediction[]> {
		...
		return (Array.isArray(body) ? body : []).map(toPrediction).filter((item): item is PlacePrediction => item !== null);
	}

	async reverse(latitude: number, longitude: number): Promise<PlacePrediction | null> {
