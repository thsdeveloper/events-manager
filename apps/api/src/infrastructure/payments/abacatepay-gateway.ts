...
export class AbacatePayGateway implements PaymentGateway {
	...
	private async request<T>(path: string, init: RequestInit): Promise<T> {
		...
		if (!response.ok || !body?.success || !body.data) {
			throw new ApiError(
				body?.error ?? 'Não foi possível processar a operação no AbacatePay.',
				502,
	...
	async getCheckout(checkoutId: string): Promise<PaymentCheckoutState> {
		...
		}>(`/checkouts/get?id=${encodeURIComponent(checkoutId)}`, { method: 'GET' });
	...
	async getPixCharge(chargeId: string): Promise<PixChargeState> {
		const charge = await this.request<{ id: string; status: string; expiresAt?: string | null }>(
		...
		return {
			id: charge.id,
			status: status as PixChargeState['status'],
