...
export async function organizerRoutes(app: FastifyInstance, options: { env: ApiEnv; clients: SupabaseClients }) {
  ...
  const grantOrganizerRole = async (userId: string) => {
  ...
  app.post('/api/organizer/request', async (request, reply) => {
    ...
    if (existing) throw new ApiError('Já existe uma solicitação ou perfil para este usuário.', 409, 'ORGANIZER_EXISTS');
    const { data, error } = await clients.admin
      .from('organizers')
      ...
      .single();
    if (error) throw error;
    if (initialOrganizerStatus === 'active') await grantOrganizerRole(auth.user.id);
  ...
  app.patch('/api/organizer/profile', async (request) => {
    ...
    const { data, error } = await clients.admin
      ...
      .maybeSingle();
    if (error) throw error;
    if (!data) throw new ApiError('Perfil de organizador não encontrado.', 404, 'ORGANIZER_NOT_FOUND');
  ...
  app.get('/api/organizer/stats', async (request) => {
    ...
    if (!eventIds.length) return { success: true, stats: { totalEvents: 0, totalRegistrations: 0, totalRevenue: 0 } };
