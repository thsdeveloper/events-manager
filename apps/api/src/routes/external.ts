...
export async function externalRoutes(app: FastifyInstance, options: { env: ApiEnv; clients: SupabaseClients }) {
  ...
  app.get('/api/places/search', async (request) => {
    ...
    if (!env.GOOGLE_PLACES_API_KEY) return { predictions: [] };
    const url = new URL('https://maps.googleapis.com/maps/api/place/autocomplete/json');
    url.searchParams.set('input', input); url.searchParams.set('language', 'pt-BR'); url.searchParams.set('types', 'geocode'); url.searchParams.set('key', env.GOOGLE_PLACES_API_KEY);
    const response = await fetch(url);
    if (!response.ok) throw new ApiError('Falha ao consultar o serviço de endereços.', 502, 'PLACES_ERROR');
    const data: any = await response.json();
    return { predictions: (data.predictions ?? []).map((item: any) => ({ placeId: item.place_id, description: item.description, mainText: item.structured_formatting?.main_text ?? item.description, secondaryText: item.structured_formatting?.secondary_text ?? null })) };
  });
