...
function paginate<T>(result: { data: T[]; total: number }, query: { page: number; limit: number }) {
	return {
		data: result.data,
		pagination: {
			page: query.page,
 ...
 */
export async function cmsRoutes(app: FastifyInstance, options: { clients: SupabaseClients; env: ApiEnv }) {
