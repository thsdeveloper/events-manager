...
async function getOrganizerAvailableBalance(clients: SupabaseClients, organizerId: string) {
...
export async function superAdminRoutes(
  ...
  app.get('/api/super-admin/overview', async (request) => {
    ...
    for (const registration of allPaid) {
      if (new Date(registration.date_created) < lastThirtyDays) continue;
  ...
  app.get('/api/super-admin/organizers', async (request) => {
    ...
    if (query.search) builder = builder.or(`name.ilike.%${query.search}%,email.ilike.%${query.search}%`);
  ...
  app.patch('/api/super-admin/organizers/:id/status', async (request) => {
    ...
    if (!before) throw new ApiError('Organizador não encontrado.', 404, 'ORGANIZER_NOT_FOUND');
  ...
  app.get('/api/super-admin/finance/transactions', async (request) => {
    ...
    const from = (query.page - 1) * query.limit;
    let builder = clients.admin.from('payment_transactions').select(
      '*,registration_id:event_registrations(id,participant_name,participant_email,event_id:events(id,title,organizer_id:organizers(id,name)))',
      { count: 'exact' },
    );
    if (query.status) builder = builder.eq('status', query.status);
    const { data, count, error } = await builder.order('date_created', { ascending: false }).range(from, from + query.limit - 1);
  ...
  app.post('/api/super-admin/finance/payouts', async (request, reply) => {
    ...
    if (input.amount > available) throw new ApiError('Saldo disponível insuficiente para este repasse.', 409, 'INSUFFICIENT_BALANCE');
