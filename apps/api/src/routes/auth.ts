...
const compatibleRegisterSchema = registerSchema.partial({ first_name: true, last_name: true }).extend({
  ...
  if (!value.first_name && !value.firstName) context.addIssue({ code: 'custom', path: ['first_name'], message: 'Nome obrigatório' });
  if (!value.last_name && !value.lastName) context.addIssue({ code: 'custom', path: ['last_name'], message: 'Sobrenome obrigatório' });
});
...
export async function authRoutes(app: FastifyInstance, options: { env: ApiEnv; clients: SupabaseClients }) {
  ...
  app.post('/api/auth/register', async (request, reply) => {
    ...
    if (error) throw new ApiError(error.message, error.status ?? 400, 'REGISTRATION_ERROR');
    if (!data.user) throw new ApiError('Não foi possível criar o usuário.', 500, 'REGISTRATION_ERROR');

    if (data.session) setSessionCookies(reply, env, data.session);
  ...
  app.post('/api/auth/login', async (request, reply) => {
    ...
    if (error || !data.session || !data.user) throw new ApiError('E-mail ou senha inválidos.', 401, 'INVALID_CREDENTIALS');
  ...
  app.post('/api/auth/refresh', async (request, reply) => {
    ...
    if (error || !data.session || !data.user) throw new ApiError('Não foi possível renovar a sessão.', 401, 'INVALID_REFRESH_TOKEN');
    setSessionCookies(reply, env, data.session);
  ...
  app.post('/api/auth/logout', async (request, reply) => {
    ...
    if (accessToken) await clients.admin.auth.admin.signOut(accessToken).catch(() => undefined);
  ...
  app.post('/api/auth/password/reset', async (request) => {
    ...
    if (error) throw new ApiError(error.message, 400, 'PASSWORD_RESET_ERROR');
    return { success: true };
  });
