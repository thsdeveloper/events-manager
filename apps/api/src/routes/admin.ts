...
async function organizerEventIds(clients: SupabaseClients, organizerId: string) {
  const { data, error } = await clients.admin.from('events').select('id').eq('organizer_id', organizerId);
  if (error) throw error;
  return (data ?? []).map((event) => event.id);
}

async function getPlatformFeePercentage(clients: SupabaseClients) {
...
export async function adminRoutes(app: FastifyInstance, options: { clients: SupabaseClients }) {
  ...
  app.get('/api/admin/ingressos', async (request) => {
    ...
    if (!eventIds.length) return { data: [], meta: { total: 0, page: query.page, pageCount: 0, perPage: 20 } };
  ...
  app.post('/api/admin/ingressos', async (request, reply) => {
    ...
    if (!eventIds.includes(input.event_id)) throw new ApiError('Evento não encontrado ou sem permissão.', 403, 'FORBIDDEN');
  ...
  app.patch('/api/admin/ingressos/:id', async (request) => {
    ...
    if (!current || (current as any).event_id.organizer_id !== context.organizer.id) throw new ApiError('Ingresso não encontrado.', 404, 'TICKET_NOT_FOUND');
  ...
  app.post('/api/admin/ingressos/:id/duplicate', async (request, reply) => {
    ...
    const { data, error } = await clients.admin
      .from('event_tickets')
      .insert({ ...copy, provider_product_id: null, event_id: (current as any).event_id.id ?? (current as any).event_id, title: `${current.title} - Cópia`, status: 'inactive', quantity_sold: 0 })
      .select('*')
      .single();
    if (error) throw error;
    return reply.code(201).send(data);
  ...
  app.get('/api/admin/participantes', async (request) => {
    ...
    if (!eventIds.length) return { data: [], meta: { total: 0, page: query.page, limit: query.limit, pageCount: 0 }, metrics: { total: 0, checkedIn: 0, pending: 0, checkInRate: 0 } };
    let builder = clients.admin.from('event_registrations').select(registrationSelect, { count: 'exact' }).in('event_id', eventIds);
    if (query.search) builder = builder.or(`participant_name.ilike.%${query.search}%,participant_email.ilike.%${query.search}%,ticket_code.ilike.%${query.search}%`);
  ...
  app.get('/api/admin/participantes/export', async (request, reply) => {
    ...
    const csv = ['Nome,Email,Telefone,Evento,Ingresso,Status,Pagamento,Check-in', ...(data ?? []).map((row: any) => [row.participant_name, row.participant_email, row.participant_phone, row.event_id?.title, row.ticket_type_id?.title, row.status, row.payment_status, row.check_in_date].map(escape).join(','))].join('\n');
