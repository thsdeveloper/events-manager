...
export class SuperAdminService {
    ...
    payments;
    now;
    constructor(repository, payments, now = () => new Date()) {
    ...
    async overview() {
        ...
        for (const event of data.events)
            performance.get(String(event.organizer_id)).events += 1;
        ...
        for (const registration of paid) {
            ...
            if (new Date(created) < lastThirtyDays)
                continue;
        ...
        return {
            metrics: {
                ...
                organizerPayable: Math.max(0, sum(succeeded, 'organizer_net') -
                    sum(data.payouts.filter((payout) => payout.status === 'completed'), 'amount')),
            },
            ...
            status: {
                organizers: Object.fromEntries(['active', 'pending', 'archived'].map((status) => [status, countBy(data.organizers, 'status', status)])),
                events: Object.fromEntries(['published', 'draft', 'cancelled', 'archived'].map((status) => [
                    status,
    ...
    async listOrganizers(query) {
        ...
        return {
            data: result.data.map((organizer) => {
                ...
                const paid = result.registrations.filter((registration) => eventOwner.get(registration.event_id) === organizer.id && registration.payment_status === 'paid');
                return {
                    ...organizer,
                    metrics: {
    ...
    async updateOrganizerStatus(actorId, id, input, context) {
    ...
    async createPayout(actorId, organizerId, amount, context) {
        ...
        try {
            ...
            await this.repository.failPayout(String(payout.id), error instanceof Error ? error.message : 'Falha no provedor de pagamentos.', this.now().toISOString());
            throw error;
        }
    }
    ...
    async updatePaymentSettings(actorId, input, context) {
    ...
    async createCategory(actorId, input, context) {
    ...
    async updateCategory(actorId, id, input, context) {
        const patch = { ...input };
        if (input.name || input.slug)
            patch.slug = await this.uniqueSlug(input.name ?? '', input.slug, id);
        const result = await this.repository.updateCategory(id, patch);
        if (!result)
            throw new ApiError('Categoria não encontrada.', 404, 'CATEGORY_NOT_FOUND');
        await this.repository.recordAudit({
            actorId,
    ...
    async deleteCategory(actorId, id, context) {
     ...
     */
    async uniqueSlug(name, provided, exceptId) {
        const base = slugify(provided?.trim() || name);
        if (!base)
            throw new ApiError('Informe um nome válido para a categoria.', 422, 'INVALID_CATEGORY_NAME');
        let candidate = base;
    ...
    async updateBrandingSettings(actorId, input, context) {
...
function payoutCreationError(error, configuration) {
    ...
    if (message.includes('PAYOUT_BELOW_MINIMUM'))
        return new ApiError(`O repasse mínimo é de R$ ${Number(configuration.minimum_payout).toFixed(2)}.`, 422, 'PAYOUT_BELOW_MINIMUM');
