...
function validateRules(value, rules) {
    for (const rule of rules?.split('|') ?? []) {
        ...
        if (name === 'email' && !z.string().email().safeParse(value).success)
            return false;
        if (name === 'url' && !z.string().url().safeParse(value).success)
            return false;
        const length = Number(argument);
        if (name === 'min' && Number.isFinite(length) && value.length < length)
            return false;
        if (name === 'max' && Number.isFinite(length) && value.length > length)
            return false;
        if (name === 'length' && Number.isFinite(length) && value.length !== length)
            return false;
    }
    return true;
}
export class SubmitForm {
    ...
    async execute(formId, userId, values) {
        ...
        for (const item of values) {
            if (submitted.has(item.field))
                throw new InvalidFormSubmission('Um campo foi enviado mais de uma vez.');
            const field = fields.get(item.field);
            if (!field)
                throw new InvalidFormSubmission('A submissão contém um campo inválido.');
            const hasValue = typeof item.value === 'string' && item.value.trim().length > 0;
            const hasFile = Boolean(item.file);
            if (hasValue === hasFile)
                throw new InvalidFormSubmission('Informe texto ou arquivo para cada campo.');
