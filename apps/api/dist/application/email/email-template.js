...
function renderBlock(block, branding) {
    switch (block.type) {
        case 'heading':
...
export function renderEmail({ branding, blocks, preheader, footerNote, footerLinks = [], }) {
		...
		<tr>
			<td style="border-top:1px solid #e5e7eb;background:#fafafa;padding:20px 32px">
				...
				<p style="margin:0 0 8px;font-size:12px">
					${links
        .map((link) => `<a href="${escapeHtml(safeUrl(link.url))}" style="margin-right:12px;color:${escapeHtml(branding.accentColor)};text-decoration:none">${escapeHtml(link.label)}</a>`)
        .join('')}
				</p>
