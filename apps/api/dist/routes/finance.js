...
async function getFinanceRows(clients, organizerId) {
...
export async function financeRoutes(app, options) {
    ...
    app.get('/api/organizer/finance/overview', async (request) => {
        ...
        const rows = (await getFinanceRows(clients, context.organizer.id)).filter((row) => {
        ...
        const gross = paid.reduce((sum, row) => sum + Number(row.total_amount ?? 0), 0);
        const serviceFees = paid.reduce((sum, row) => sum + Number(row.platform_fee ?? 0) + Number(row.provider_fee ?? 0), 0);
        const net = paid.reduce((sum, row) => sum + Math.max(0, Number(row.payment_amount ?? 0) - Number(row.provider_fee ?? 0) - (Number(row.total_amount ?? 0) > Number(row.payment_amount ?? 0) ? 0 : Number(row.platform_fee ?? 0))), 0);
        const totalTransactions = paid.length;
        return { metrics: { gross, serviceFees, net, ticketsSold: paid.reduce((sum, row) => sum + Number(row.quantity ?? 0), 0), totalTransactions, averageTicket: totalTransactions ? gross / totalTransactions : 0, pendingCount: rows.filter((row) => row.payment_status === 'pending').length, refundedCount: rows.filter((row) => row.payment_status === 'refunded').length, allRegistrations: rows.length, allGross: rows.reduce((sum, row) => sum + Number(row.total_amount ?? 0), 0) } };
    });
    app.get('/api/organizer/finance/transactions', async (request) => {
        ...
        registrations = registrations.filter((row) => (!query.event_id || row.event_id.id === query.event_id) && (!query.search || `${row.participant_name} ${row.participant_email}`.toLowerCase().includes(query.search.toLowerCase())));
        const ids = registrations.map((row) => row.id);
        if (!ids.length)
            return { data: [], pagination: { page: query.page, limit: query.limit, total: 0, pageCount: 0 }, summary: { status: {} } };
        ...
        if (query.date_from)
            builder = builder.gte('date_created', query.date_from);
        if (query.date_to)
            builder = builder.lte('date_created', query.date_to);
    ...
    app.get('/api/organizer/analytics', async (request) => {
        ...
        const eventIds = [...new Set((query.event_id ? [query.event_id] : allRows.map((row) => row.event_id?.id)).filter(Boolean))];
        const { data: events, error: eventsError } = await clients.admin
            .from('events')
        ...
        const totalRevenue = paid.reduce((sum, row) => sum + Number(row.payment_amount ?? 0), 0);
        ...
        const previousRevenue = previousPaid.reduce((sum, row) => sum + Number(row.payment_amount ?? 0), 0);
        const checkinRate = paid.length ? (checkedIn / paid.length) * 100 : 0;
        const previousRate = previousPaid.length ? (previousPaid.filter((row) => row.check_in_date).length / previousPaid.length) * 100 : 0;
        const percentageChange = (current, previous) => previous ? ((current - previous) / previous) * 100 : 0;
        ...
        const ticketPerformance = (tickets ?? []).map((ticket) => {
        ...
        const installmentAlerts = ['overdue', 'upcoming', 'defaulted'].map((type) => {
            const matching = (installments ?? []).filter((item) => {
                ...
                if (type === 'overdue')
                    return item.status === 'overdue' || (item.status === 'pending' && due < now);
                if (type === 'upcoming')
                    return item.status === 'pending' && due >= now && due <= now + 7 * 86_400_000;
                return false;
            });
            return { type, count: matching.length, amount: matching.reduce((sum, item) => sum + Number(item.amount ?? 0), 0) };
        ...
        const activeEvents = (events ?? []).filter((event) => event.status === 'published' || event.status === 'draft').map((event) => {
            const eventTickets = (tickets ?? []).filter((ticket) => ticket.event_id === event.id);
            const ticketsSold = eventTickets.reduce((sum, ticket) => sum + Number(ticket.quantity_sold ?? 0), 0);
            const ticketsTotal = eventTickets.reduce((sum, ticket) => sum + Number(ticket.quantity ?? 0), 0);
            const rate = ticketsTotal ? (ticketsSold / ticketsTotal) * 100 : 0;
            const revenue = paid.filter((row) => row.event_id?.id === event.id).reduce((sum, row) => sum + Number(row.payment_amount ?? 0), 0);
        ...
        return {
            kpi: {
                ...
                ticketsSold: paid.reduce((sum, row) => sum + Number(row.quantity ?? 1), 0),
                ticketsTotal: (tickets ?? []).reduce((sum, ticket) => sum + Number(ticket.quantity ?? 0), 0),
    ...
    app.get('/api/organizer/finance/export', async (request, reply) => {
        ...
        const csv = ['Data,Evento,Participante,Email,Status,Bruto,Taxas,Líquido', ...rows.map((row) => [row.date_created, row.event_id?.title, row.participant_name, row.participant_email, row.payment_status, row.total_amount, row.service_fee, row.payment_amount].map((value) => `"${String(value ?? '').replaceAll('"', '""')}"`).join(','))].join('\n');
