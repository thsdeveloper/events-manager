...
export async function superAdminRoutes(app, options) {
    ...
    app.patch('/api/super-admin/settings/payments', async (request) => {
        const admin = await requireSuperAdmin(request, auth);
        return service.updatePaymentSettings(admin.user.id, paymentSettingsSchema.parse(request.body), auditContext(request));
    });
    ...
    app.patch('/api/super-admin/settings/branding', async (request) => {
        const admin = await requireSuperAdmin(request, auth);
        return service.updateBrandingSettings(admin.user.id, brandingSettingsSchema.parse(request.body), auditContext(request));
    });
}
