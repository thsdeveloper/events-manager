...
export class SupabaseAdminRepository {
    ...
    async getPlatformFeePercentage() {
        const data = (await this.getConfiguration());
        return Number(data.platform_fee_percentage ?? 0);
    }
    async listOwnedEventIds(organizerId) {
    ...
    async listEventOptions(organizerId) {
    ...
    async listTickets(eventIds, query) {
    ...
    async duplicateTicket(input) {
        const { data, error } = await this.clients.admin.from('event_tickets').insert(input).select('*').single();
        if (error)
            throw error;
        return data;
    }
    async listParticipantFilterOptions(eventIds) {
        if (!eventIds.length)
    ...
    async listParticipants(eventIds, query) {
        let builder = this.clients.admin
        ...
        if (query.search) {
            ...
            if (search)
                builder = builder.or(`participant_name.ilike.%${search}%,participant_email.ilike.%${search}%,ticket_code.ilike.%${search}%`);
        }
    ...
    async findOwnedRegistration(organizerId, id) {
        const { data, error } = await this.clients.admin
            .from('event_registrations')
    ...
    async cancelRegistration(organizerId, id, reason) {
