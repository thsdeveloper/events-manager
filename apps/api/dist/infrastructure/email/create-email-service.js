 ...
 * reads it directly.
 */
export function createEmailService(env, clients) {
    const branding = new CachedEmailBranding(createEmailBrandingReader(clients), env.WEB_URL);
    const email = new EmailService(new SupabaseEmailDeliveryRepository(clients), new NodemailerGateway({
        host: env.SMTP_HOST,
        port: env.SMTP_PORT,
        secure: env.SMTP_SECURE,
        user: env.SMTP_USER,
        password: env.SMTP_PASSWORD,
    }), env.EMAIL_FROM, branding);
    return { branding, email };
}
//# sourceMappingURL=create-email-service.js.map